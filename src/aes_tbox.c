...
/* AES-128 implementation using TBOXes */
...
static uint32_t expanded_key[44]; // expanded key
...
// shifts a number arithmetically right by n bytes
uint32_t arshift(uint32_t w, int n) {
    n = (n % 4) * 8; // (0-3) * 8
    return (w >> n) | (w << (32 - n));
}
...
void shiftRows(t_state s) {
    ...
    for (int i = 0; i < 4; ++i)
        bak[i] = s[i];
    for (int i = 0; i < 4; ++i)
        s[i] = word(wbyte(bak[i], 0), wbyte(bak[(i + 1) % 4], 1), wbyte(bak[(i + 2) % 4], 2), wbyte(bak[(i + 3) % 4], 3));
}
...
*/
void expandKey(uint8_t k[16], uint32_t expanded_key[44]) {
	uint32_t tmp;
    uint8_t rcon = 1;
    int i4 = 0;
    for (int i = 0; i < 4; ++i, i4 = i * 4) // copy key to first 4 words
        expanded_key[i] = word(k[i4], k[i4 + 1], k[i4 + 2], k[i4 + 3]);

    for (int i = 4; i < 44; ++i) {
        tmp = expanded_key[i - 1];
        if (i % 4 == 0) {
            tmp = subWord(arshift(tmp, 1)) ^ rcon;
            rcon = xtime(rcon);
        }
        expanded_key[i] = expanded_key[i - 4] ^ tmp;
    }
}

/* Adding expanded round key (prepared before) */
void addRoundKey(t_state s, uint32_t expanded_key[], short round) {
	for (int i = 0, r4 = round * 4; i < 4; ++i)
        s[i] ^= expanded_key[r4 + i];
}

void tboxRound(t_state s) {
    t_state bak;
    for (int i = 0; i < 4; ++i)
        bak[i] = s[i];
    for (int i = 0; i < 4; ++i)
        s[i] = TBOX_0[wbyte(bak[i], 0)] ^ TBOX_1[wbyte(bak[(i + 1) % 4], 1)] ^ TBOX_2[wbyte(bak[(i + 2) % 4], 2)] ^ TBOX_3[wbyte(bak[(i + 3) % 4], 3)];
}

/* The prefetch function pulled from OpenSSL aes_x86core.c
static void prefetch256(const void *table)
{
    volatile uint32_t *t=(void *)table,ret;
    unsigned long sum;
    int i;
...
void aes(uint8_t *in, uint8_t *out)
{
	...
	t_state state;
    for (int i = 0, i4 = 0; i < 4; ++i, i4 = i * 4)
	    state[i] = word(in[i4],  in[i4 + 1],  in[i4 + 2],  in[i4 + 3]);
